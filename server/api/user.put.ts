...
export default defineEventHandler(async (event) => {
    ...
    if (userDoc.exists) {
        setResponseStatus(event, 202, 'User is already added.')
    }
    else {
        setResponseStatus(event, 200, 'User added.')
        await userRef.set(User.toJson(User.fromJson({ uid: id, isAdmin: false })))
    }
})
